...
const SignupForm = ({ users, setUsers, addUser }) => {
  ...
  // 1. onChange => 5가지의 인풋에 반응할 수 있는 함수 만들기 각각의 인풋의 값을 변경할 때마다 해당하는 값만 변경할 수 있는함수를 만드세요.
  ...
  const onSubmit = () => {
    // early return 예외처리
    if (email.length === 0) {
      return alert("emailㄱㄱ");
    }
    if (password.length === 0) {
      return alert("passwordㄱㄱ");
    }
    if (name.length === 0) {
      return alert("nameㄱㄱ");
    }
    if (mobile.length === 0) {
      return alert("mobileㄱㄱ");
    }
    if (address.length === 0) {
      return alert("addressㄱㄱ");
    }
